...
MString::MString(const char* s) noexcept {
	len = strlen(s);
	std::unique_ptr<char[]> temp = std::move(data); // 将data的所有权转,超出范围自动释放
...
MString& MString::operator=(MString&& other) noexcept {
	if (this != &other) {
		data = std::move(other.data); // 使用std::move转移智能指针的所有权
		len = other.len;
		other.len = 0; // 重置源对象的长度
	}
	return *this;
...
int MString::compareTo(const MString& other) const {
	return strcmp(data.get(), other.data.get());
}
...
std::vector<MString> MString::split(const char* delimiter) const {
	...
	char* token = strtok(data.get(), delimiter);
	while (token != nullptr) {
...
MString MString::join(const std::vector<MString>& strings, const MString& separator) {
	std::string result;
	if (!strings.empty()) {
		result.append(strings[0].data.get());  // 添加第一个元素
		for (size_t i = 1; i < strings.size(); i++) {
			result.append(separator.data.get());  // 添加分隔符
			result.append(strings[i].data.get());  // 添加当前元素
		}
	}
...
int MString::find(const char* substr, size_t startPos) const {
	...
	// 从 startPos 开始进行查找
	size_t i = startPos;
	while (i <= len - patternLen) {
		size_t j = patternLen - 1;

		// 从后向前匹配
		while (j >= 0 && data[i + j] == substr[j]) {
			--j;
		}
		...
		// 根据坏字符规则，计算跳跃距离
		size_t badCharShift = j - badChar[static_cast<unsigned char>(data[i + j])];
...
int MString::rfind(const char* substr, size_t startPos) const {
	...
	// 默认从字符串末尾开始查找
	if (startPos == std::string::npos || startPos >= len) {
		startPos = len - 1;
	}
	...
	for (size_t i = startPos; i >= patternLen - 1; --i) {
		...
		for (size_t j = 0; j < patternLen; ++j) {
			if (data[i - j] != substr[patternLen - j - 1]) {
				match = false;
				break;
			}
		}
...
MString MString::mid(size_t pos, size_t n) const {
	...
	size_t numChars = (pos + n > len) ? len - pos : n;
	std::unique_ptr<char[]> sub(new char[numChars + 1]);
	std::memcpy(sub.get(), data.get() + pos, numChars);
	sub[numChars] = '\0';
...
MString MString::toUpperCase() const {
	std::unique_ptr<char[]> upper(new char[len + 1]);
	for (size_t i = 0; i < len; i++) {
		upper[i] = std::toupper(data[i]);
	}
	upper[len] = '\0';
...
MString MString::toLowerCase() const {
	std::unique_ptr<char[]> lower(new char[len + 1]);
	for (size_t i = 0; i < len; i++) {
		lower[i] = std::tolower(data[i]);
	}
	lower[len] = '\0';
...
bool MString::isUtf8Encoding(const char* str) {
	...
	//0x00-0x7F为ASCII码范围
	for (int uIndex = 0; uIndex < len; ++uIndex) {
		single = str[uIndex];
...
MString MString::getHexString() const {
	...
	size_t hexLen = len * 3 - 1; // two hex digits per byte plus spaces
...
bool MString::operator==(const MString& other) const {
	...
	// 然后逐字符比较
	return strcmp(this->data.get(), other.data.get()) == 0;
}
